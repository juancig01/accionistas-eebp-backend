...
public interface RespuestasRepository extends JpaRepository<Respuestas, Integer> {
    ...
    @Query(value = "SELECT p.id_pregunta, p.pregunta, op.id_opc_respuesta, op.opcion_respuesta, COUNT(r.id_opc_respuesta) AS num_votos " +
            "FROM preguntas p " +
            ...
            "GROUP BY p.id_pregunta, p.pregunta, op.id_opc_respuesta, op.opcion_respuesta", nativeQuery = true)
